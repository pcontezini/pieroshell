...
namespace internal {
  ...
  int NativesCollection<CORE>::GetIndex(const char* name) {
    ...
    if (strcmp(name, "json") == 0) return 3;
    if (strcmp(name, "regexp") == 0) return 4;
  ...
  Vector<const char> NativesCollection<CORE>::GetScriptSource(int index) {
    if (index == 0) return Vector<const char>(debug, 37161);
    if (index == 1) return Vector<const char>(mirror, 32307);
    if (index == 2) return Vector<const char>(date, 23672);
    if (index == 3) return Vector<const char>(json, 5813);
    if (index == 4) return Vector<const char>(regexp, 8944);
    if (index == 5) return Vector<const char>(runtime, 10111);
    if (index == 6) return Vector<const char>(v8natives, 10503);
    if (index == 7) return Vector<const char>(array, 22774);
    if (index == 8) return Vector<const char>(string, 17502);
    if (index == 9) return Vector<const char>(uri, 8571);
    if (index == 10) return Vector<const char>(math, 4017);
    if (index == 11) return Vector<const char>(messages, 15627);
    if (index == 12) return Vector<const char>(apinatives, 2095);
  ...
  Vector<const char> NativesCollection<CORE>::GetScriptName(int index) {
    ...
    if (index == 3) return Vector<const char>("native json.js", 14);
    if (index == 4) return Vector<const char>("native regexp.js", 16);
